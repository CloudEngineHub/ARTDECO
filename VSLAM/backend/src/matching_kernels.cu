...
{
  // batch index
  const uint64_t n = blockIdx.x * blockDim.x + threadIdx.x; // pixel location
  const uint64_t b = blockIdx.y; // batch location
...
std::vector<torch::Tensor> iter_proj_cuda(
    ...
    torch::Tensor p_init,   // b (h*w) 2 初始的2中的点到1的像素坐标的匹配关系
    const int max_iter,
    const float lambda_init,
    const float cost_thresh)
{
