    ...
    const float Q_thresh)
{
  ...
  const int block_id = blockIdx.x; // edge id
  const int thread_id = threadIdx.x;
 
  const int num_points = Xs.size(1);
 
  int ix = static_cast<int>(ii[block_id]); // 这个关键帧的unique index（在Twc Xs Cs）
  int jx = static_cast<int>(jj[block_id]); // 这个关键帧的unique index（在Twc Xs Cs）

 // 1. 读取当前边的位姿并且计算出他们的相对位姿
  __shared__ float ti[3], tj[3], tij[3];
  __shared__ float qi[4], qj[4], qij[4];
  __shared__ float si[1], sj[1], sij[1];
  ...
  __syncthreads();
  // 2. 初始化一些变量，当前block中的256个线程循环一次处理当前边的256个点
  // //points
  float Xi[3];
  float Xj[3];
  float Xj_Ci[3];
  ...
  const float delta_thresh)
{
  ...
  // Setup indexing 升序排序
  torch::Tensor unique_kf_idx = get_unique_kf_idx(ii, jj);
  // For edge construction ii和jj在unique_kf_idx中的位置
  std::vector<torch::Tensor> inds = create_inds(unique_kf_idx, 0, ii, jj);
  torch::Tensor ii_edge = inds[0]; // ii在unique_kf_idx中的位置
  torch::Tensor jj_edge = inds[1]; // ii在unique_kf_idx中的位置
  // For linear system indexing (pin=2 because fixing first two poses)
  std::vector<torch::Tensor> inds_opt = create_inds(unique_kf_idx, num_fix, ii, jj);
  torch::Tensor ii_opt = inds_opt[0]; // ii在unique_kf_idx中去掉前两帧的位置
  torch::Tensor jj_opt = inds_opt[1]; // jj在unique_kf_idx中去掉前两帧的位置
